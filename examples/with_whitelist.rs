/* fn main() {
  ...
  let message_with_all_tags = twitch::MessageParts::parse(data).unwrap();
  let message_with_some_tags = twitch::MessageParts::parse_with_whitelist(
    data,
    twitch::whitelist!(Badges, Color, DisplayName, Emotes),
  )
  .unwrap();

  for (tag, value) in message_with_all_tags.tags().iter().flat_map(|m| m.iter()) {
    print!("{tag}={value};");
  }
  println!();
  ...
  for (tag, value) in message_with_some_tags.tags().iter().flat_map(|m| m.iter()) {
    print!("{tag}={value};");
  }
  println!();
}
